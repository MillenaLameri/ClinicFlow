...
public sealed class DoctorsController : ControllerBase
{
    ...
    public DoctorsController(ClinicFlowDbContext dbContext)
    {
    ...
    [ProducesResponseType(
        typeof(IReadOnlyCollection<DoctorResponse>),
        StatusCodes.Status200OK
    )]
    public async Task<ActionResult<IReadOnlyCollection<DoctorResponse>>> GetAll(
    ...
    )
    {
        ...
        {
            query = query.Where(
                doctor => doctor.SpecialtyId == specialtyId.Value
            );
        }
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(doctor =>
                ...
                || doctor.CrmNumber.ToLower().Contains(normalizedSearch)
                || doctor.Email.ToLower().Contains(normalizedSearch)
            );
        }
    ...
    )
    {
        ...
        {
            return SpecialtyNotFound(request.SpecialtyId);
        }

        var normalizedCrmNumber = request.CrmNumber
            .Trim()
            .ToUpperInvariant();

        var normalizedCrmState = request.CrmState
            .Trim()
            .ToUpperInvariant();
        ...
        var crmAlreadyExists = await _dbContext.Doctors.AnyAsync(
            doctor =>
                doctor.CrmNumber == normalizedCrmNumber
                && doctor.CrmState == normalizedCrmState,
            cancellationToken
        );
        ...
        var emailAlreadyExists = await _dbContext.Doctors.AnyAsync(
            doctor => doctor.Email == normalizedEmail,
            cancellationToken
        );

        if (emailAlreadyExists)
        {
            return DoctorEmailConflict(normalizedEmail);
        }
        ...
        {
            return InvalidDoctorData(exception.Message);
        }
            ...
            when (IsUniqueConstraintViolation(exception))
        {
            return Problem(
                ...
                title: "Médico já cadastrado.",
                detail:
                    "Já existe um médico utilizando o CRM ou e-mail informado."
            );
        }

        var response = DoctorResponse.FromEntity(
            doctor,
            specialty
        );
    ...
    private ObjectResult DoctorNotFound(Guid id)
    {
        return Problem(
            statusCode: StatusCodes.Status404NotFound,
            title: "Médico não encontrado.",
            detail: $"Não foi encontrado um médico com o ID '{id}'."
        );
    }
    ...
    private ObjectResult DoctorCrmConflict(
        string crmNumber,
        string crmState
    )
    {
    ...
    private ObjectResult DoctorEmailConflict(string email)
    {
        return Problem(
            statusCode: StatusCodes.Status409Conflict,
    ...
    private ObjectResult InvalidDoctorData(string detail)
    {
        return Problem(
            statusCode: StatusCodes.Status400BadRequest,
            title: "Dados do médico inválidos.",
            detail: detail
        );
    }
